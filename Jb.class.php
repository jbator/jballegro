...
class Jb extends Module
{
        ...
        foreach ($result as $row)
        {
            try
            {
                ...
                if (!$data)
                {
                    ...
                    if (!$data)
                        $data = $this->allegro->getMyAccountData('not_sold', array($aid));
                    $closed = true;
                }
                //echo 'cl ' . $closed . '<br />';
                //echo '<pre>';
    ...
    public function getStateSelectForCategory($id)
    {
        if ($id)
        {
            $cat = $query = 'SELECT * FROM ' . _DB_PREFIX_ . 'allegro_cat where id = ' . $id . ';';
            $catData = Db::getInstance()->ExecuteS($query);;
            
            $parentId = $catData[0]['parent'];
            
            $query = 'SELECT * FROM ' . _DB_PREFIX_ . 'allegro_field where `sell-form-title` = "Stan" and `sell-form-cat` = ' . $id . ';';
            ...
            else $data = $this->getStateSelectForCategory($parentId);
